...
try:
    from wan_va.ledger.neural import NeuralLedgerHead
except ImportError:  # Script-style execution with ``wan_va`` on sys.path.
    from ledger.neural import NeuralLedgerHead
...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
                 ...
                 attn_mode="torch",
                 enable_ledger=False,
                 ledger_hidden_dim=256,
                 ledger_num_claim_slots=16,
                 ledger_num_relations=12,
                 ledger_max_rollback_steps=16,
                 ledger_num_repair_actions=8,
                 ledger_delta_dim=None,
                 ledger_num_heads=8,
                 ledger_dropout=0.0,
        ...
        # Ledger-WAM is an opt-in side head.  Keeping it disabled preserves the
        # exact parameterization and behavior of released LingBot-VA weights.
        # When enabled, the structured ledger context is injected into action
        # tokens through an adapter instead of extending the causal token
        # sequence, so the existing FlexAttention mask and KV cache stay valid.
        self.enable_ledger = bool(enable_ledger)
        if self.enable_ledger:
            self.ledger_head = NeuralLedgerHead(
                latent_channels=in_channels,
                action_dim=action_dim,
                text_dim=text_dim,
                hidden_dim=ledger_hidden_dim,
                num_claim_slots=ledger_num_claim_slots,
                num_relations=ledger_num_relations,
                num_rollback_steps=ledger_max_rollback_steps,
                num_repair_actions=ledger_num_repair_actions,
                delta_dim=ledger_delta_dim,
                num_heads=ledger_num_heads,
                dropout=ledger_dropout,
                num_claim_types=ledger_num_claim_types,
                num_subjects=ledger_num_subjects,
                num_objects=ledger_num_objects,
                num_preconditions=ledger_num_preconditions,
                num_effects=ledger_num_effects,
            )
            self.ledger_condition_proj = nn.Sequential(
                nn.LayerNorm(ledger_hidden_dim),
                nn.Linear(ledger_hidden_dim, inner_dim),
            )
            nn.init.zeros_(self.ledger_condition_proj[1].weight)
            nn.init.zeros_(self.ledger_condition_proj[1].bias)
        else:
            self.ledger_head = None
            self.ledger_condition_proj = None

    def _condition_action_tokens(
        self, hidden_states, ledger_context, ledger_context_mask=None):
        """Inject a per-sample ledger summary without changing token layout."""
        if ledger_context is None or self.ledger_condition_proj is None:
            return hidden_states
        if ledger_context.ndim != 2:
            raise ValueError(
                "ledger_context must have shape [batch, hidden_dim], got "
                f"{tuple(ledger_context.shape)}"
            )
        batch_size = hidden_states.shape[0]
        context_batch = ledger_context.shape[0]
        if context_batch != batch_size:
            if context_batch <= 0 or batch_size % context_batch != 0:
                raise ValueError(
                    "ledger_context batch must divide action-token batch: "
                    f"{context_batch} vs {batch_size}"
                )
            ledger_context = ledger_context.repeat(batch_size // context_batch, 1)
        condition = self.ledger_condition_proj(
            ledger_context.to(
                device=hidden_states.device,
                dtype=self.ledger_condition_proj[1].weight.dtype,
            )
        )
        if ledger_context_mask is not None:
            mask = ledger_context_mask.to(
                device=condition.device, dtype=condition.dtype).reshape(-1, 1)
            if mask.shape[0] != condition.shape[0]:
                if condition.shape[0] % mask.shape[0] != 0:
                    raise ValueError(
                        "ledger context-mask batch does not match action batch")
                mask = mask.repeat(condition.shape[0] // mask.shape[0], 1)
            condition = condition * mask
        return hidden_states + condition.to(hidden_states.dtype)[:, None, :]

    def forward_ledger(self, input_dict):
        """Predict structured causal claims from observed state history."""
        if self.ledger_head is None:
            raise RuntimeError(
                "Ledger-WAM is disabled; load the model with enable_ledger=True"
            )
        ledger_dtype = next(self.ledger_head.parameters()).dtype
        counterfactual_actions = input_dict.get("counterfactual_actions")
        if counterfactual_actions is not None:
            counterfactual_actions = counterfactual_actions.to(ledger_dtype)
        previous_claim_slots = input_dict.get("previous_claim_slots")
        if previous_claim_slots is not None:
            previous_claim_slots = previous_claim_slots.to(ledger_dtype)
        previous_claim_mask = input_dict.get("previous_claim_mask")
        masks = dict(input_dict.get("masks") or {})
        if "text" not in masks:
            # T5 embeddings are padded with exact zeros by preprocessing.  The
            # projection has bias, so padding must be masked before attention.
            masks["text"] = input_dict["text_emb"].detach().abs().any(dim=-1)
        return self.ledger_head(
            latents=input_dict["latents"].to(ledger_dtype),
            actions=input_dict["actions"].to(ledger_dtype),
            text_emb=input_dict["text_emb"].to(ledger_dtype),
            counterfactual_actions=counterfactual_actions,
            masks=masks,
            previous_claim_slots=previous_claim_slots,
            previous_claim_mask=previous_claim_mask,
        )
    ...
    def forward_train(self, input_dict):
        ...
        input_dict['latent_dict']['noisy_latents'] = input_dict['latent_dict']['noisy_latents'].to(model_dtype)
        input_dict['latent_dict']['latent'] = input_dict['latent_dict']['latent'].to(model_dtype)
        input_dict['action_dict']['noisy_latents'] = input_dict['action_dict']['noisy_latents'].to(model_dtype)
        input_dict['action_dict']['latent'] = input_dict['action_dict']['latent'].to(model_dtype)
        ...
        ledger_outputs = None
        ledger_context = None
        ledger_context_mask = None
        if self.ledger_head is not None and input_dict.get("ledger_dict") is not None:
            ledger_dict = input_dict["ledger_dict"]
            has_temporal_prefix = (
                ledger_dict["latents"].shape[2] > 1
                and ledger_dict["actions"].shape[2] > 1
            )
            if (
                self.training
                and ledger_dict.get("previous_claim_slots") is None
                and has_temporal_prefix
            ):
                # Only explicit multi-frame history supplies a meaningful
                # recurrent training pair.  Legacy one-frame segments train
                # the no-history path directly, matching the first online
                # server update instead of teaching an updater to copy the
                # same detached state.
                previous_dict = dict(ledger_dict)
                previous_dict["latents"] = ledger_dict["latents"][:, :, :-1]
                previous_dict["actions"] = ledger_dict["actions"][:, :, :-1]
                previous_dict["counterfactual_actions"] = None
                previous_masks = dict(ledger_dict.get("masks") or {})
                previous_masks.pop("latent", None)
                previous_masks.pop("action", None)
                previous_dict["masks"] = previous_masks
                with torch.no_grad():
                    previous_claim_slots = self.forward_ledger(
                        previous_dict)["claim_slots"].detach()
                ledger_dict = dict(ledger_dict)
                ledger_dict["previous_claim_slots"] = previous_claim_slots
                if ledger_dict.get("previous_claim_mask") is None:
                    ledger_dict["previous_claim_mask"] = previous_masks.get(
                        "claim")
            ledger_outputs = self.forward_ledger(ledger_dict)

            recurrent_next = ledger_dict.get("recurrent_next")
            if self.training and recurrent_next is not None:
                # The next observation is used only to teach the recurrent
                # state updater to reproduce a fresh no-history ledger.  Its
                # context is never injected into the current action stream.
                with torch.no_grad():
                    recurrent_teacher = self.forward_ledger(recurrent_next)
                recurrent_student_dict = dict(recurrent_next)
                recurrent_student_dict["previous_claim_slots"] = (
                    ledger_outputs["claim_slots"].detach()
                )
                recurrent_student = self.forward_ledger(
                    recurrent_student_dict)
                recurrent_error = F.smooth_l1_loss(
                    recurrent_student["claim_slots"],
                    recurrent_teacher["claim_slots"].detach(),
                    reduction="none",
                )
                # Keep some signal on null slots so the updater learns how a
                # new claim can appear, while focusing on teacher-active slots.
                recurrent_weight = (
                    0.25 + 0.75 * recurrent_teacher["presence"].detach()
                ).unsqueeze(-1)
                ledger_outputs["recurrent_consistency_loss"] = (
                    recurrent_error * recurrent_weight
                ).mean()
            ledger_context = ledger_outputs["ledger_context"]
            ledger_context_mask = ledger_dict.get("context_mask")
        ...
        action_hidden_states = self._condition_action_tokens(
        ...
        condition_action_hidden_states = self._condition_action_tokens(
        ...
        if ledger_outputs is not None:
            return latent_hidden_states, action_hidden_states, ledger_outputs
    ...
    def forward(
        ...
        """
        if ledger_mode:
            return self.forward_ledger(input_dict)
        if train_mode:
        ...
        if action_mode:  # action input emb
            ...
            latent_hidden_states = self._condition_action_tokens(
                latent_hidden_states, input_dict.get("ledger_context"))
