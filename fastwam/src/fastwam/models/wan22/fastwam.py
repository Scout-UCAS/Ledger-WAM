...
class FastWAM(torch.nn.Module):
    ...
    def from_wan22_pretrained(
        ...
        if extra_init_kwargs is None:
            extra_init_kwargs = {}
        if not isinstance(extra_init_kwargs, dict):
            raise ValueError("`extra_init_kwargs` must be a dict when provided.")
        model = cls(
    ...
    def training_loss(self, sample, tiled: bool = False):
        ...
        auxiliary_loss, auxiliary_metrics = self.auxiliary_training_loss(
            sample=sample,
            state_tokens=tokens_out["action"],
            video_state_tokens=tokens_out["video"],
            action=action,
        )
    ...
    def auxiliary_training_loss(
        ...
        """Extension point for structured heads that share Fast-WAM features."""

        return state_tokens.sum() * 0.0, {}
    ...
    def infer_action(
        ...
        for step_t_action, step_delta_action in zip(infer_timesteps_action, infer_deltas_action):
            ...
            if return_features:
                pred_action_posi, last_action_features = prediction
            else:
        ...
        if return_features:
            if last_action_features is None:
                raise RuntimeError("Action inference produced no feature tokens.")
            output["action_features"] = last_action_features
            output["video_features"] = video_pre["tokens"]
        return output
    ...
    def save_checkpoint(self, path, optimizer=None, step=None):
        ...
        auxiliary_state = self.auxiliary_checkpoint_state()
        if auxiliary_state:
            payload["auxiliary"] = auxiliary_state
    ...
    def load_checkpoint(self, path, optimizer=None):
        ...
        self.load_auxiliary_checkpoint_state(payload.get("auxiliary"))
    ...
    def load_auxiliary_checkpoint_state(self, state: Optional[dict[str, Any]]) -> None:
        if state:
            logger.warning("Ignoring unsupported auxiliary checkpoint keys: %s", sorted(state))

    def set_train_mode_for_training(self) -> None:
        """Freeze non-DiT components while keeping trainable policy heads enabled."""
