...
from ..ledger_annotations import LedgerAnnotationStore
from ..ledger_evidence import WeakCausalAnnotator
...
class RobotVideoDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        ledger_annotations_path: Optional[str] = None,
        ledger_num_claims: int = 8,
        ledger_num_entities: int = 8,
        ledger_num_repair_actions: int = 4,
        ledger_strict_coverage: bool = True,
        ledger_auto_annotate: bool = False,
        ledger_temporal_steps: int = 4,
    ):
        ...
        self.ledger_annotations = (
            None
            if ledger_annotations_path is None
            else LedgerAnnotationStore(
                ledger_annotations_path,
                num_claims=ledger_num_claims,
                num_entities=ledger_num_entities,
                num_repair_actions=ledger_num_repair_actions,
                strict_coverage=ledger_strict_coverage,
            )
        )
        self.ledger_auto_annotator = (
            WeakCausalAnnotator(
                num_claims=int(ledger_num_claims),
                num_entities=int(ledger_num_entities),
                num_repair_actions=int(ledger_num_repair_actions),
                steps=int(ledger_temporal_steps),
            )
            if ledger_auto_annotate
            else None
        )
    ...
    def _get(self, idx):
        ...
        if self.ledger_auto_annotator is not None:
            data.update(
                self.ledger_auto_annotator.annotate(
                    video=video,
                    proprio=proprio,
                    action=action,
                    valid_mask=~sample["action_is_pad"].bool(),
                )
            )
        if self.ledger_annotations is not None:
            data.update(
                self.ledger_annotations.get(
                    int(sample["idx"]),
