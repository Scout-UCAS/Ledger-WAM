...
def create_ledger_wam(
    ...
    """Instantiate Ledger-WAM while preserving Fast-WAM's pretrained backbone path."""

    from .models.wan22.ledger_wam import LedgerWAM

    def _plain_dict(value, name: str, *, required: bool = False):
        if isinstance(value, DictConfig):
            value = OmegaConf.to_container(value, resolve=True)
        if value is None:
            if required:
                raise ValueError(f"`{name}` is required for Ledger-WAM.")
            return {}
        if not isinstance(value, dict):
            raise ValueError(f"`{name}` must be dict-like, got {type(value)}")
        return value

    video_dit_config = _plain_dict(video_dit_config, "video_dit_config", required=True)
    action_dit_config = _plain_dict(action_dit_config, "action_dit_config")
    video_scheduler = _plain_dict(video_scheduler, "video_scheduler")
    action_scheduler = _plain_dict(action_scheduler, "action_scheduler", required=True)
    loss = _plain_dict(loss, "loss")
    ledger = _plain_dict(ledger, "ledger", required=True)
    ledger_loss = _plain_dict(ledger_loss, "ledger_loss")
    planner = _plain_dict(planner, "planner")
    world_predictor = _plain_dict(world_predictor, "world_predictor")
    task_graph = _plain_dict(task_graph, "task_graph")
    ...
    return LedgerWAM.from_wan22_pretrained(
        device=device,
        torch_dtype=model_dtype,
        model_id=model_id,
        tokenizer_model_id=tokenizer_model_id,
        ...
        extra_init_kwargs={
            ...
            "ledger_loss_config": ledger_loss,
            "planner_config": planner,
            "world_predictor_config": world_predictor,
            "task_graph_config": task_graph,
        },
    )
