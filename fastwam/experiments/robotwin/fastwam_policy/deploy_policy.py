...
from experiments.common import SimulatorEventParser, apply_joint_repair
...
class WorldActionRobotWinPolicy:
    def __init__(
        ...
        repair_skill_config: Optional[Dict[str, Any]] = None,
    ) -> None:
        ...
        self.event_parser = SimulatorEventParser()
    ...
    def _infer_action_chunk(self, observation: Dict[str, Any], instruction: str) -> np.ndarray:
        ...
        if hasattr(self.model, "ledger"):
            evidence, _ = self.event_parser.observe(
                observation,
                image=observation["observation"]["head_camera"]["rgb"],
            )
            infer_kwargs["ledger_evidence"] = evidence
        ...
        execution_horizon = int(pred.get("execution_horizon", action_tensor.shape[0]))
        execution_horizon = max(1, min(execution_horizon, action_tensor.shape[0]))
        action_tensor = action_tensor[:execution_horizon]
        action_chunk = self._denormalize_action(action_tensor)[0]  # [T, D]
        action_chunk = apply_joint_repair(
            action_chunk,
            pred.get("planner", {}).get("repair_name"),
            current_qpos=state_vector,
            config=self.repair_skill_config,
        )
        self.event_parser.last_action = np.asarray(action_chunk[0], dtype=np.float32)
        return action_chunk
    ...
    def reset(self) -> None:
        ...
        self.event_parser.reset()
        self.episode_count += 1
        self.step_count = 0
        self.reset_timing_rollout()

    def get_ledger_episode_metrics(self) -> Optional[Dict[str, Any]]:
        if not hasattr(self.model, "get_ledger_episode_metrics"):
            return None
        metrics = self.model.get_ledger_episode_metrics(reset=False)
        metrics["simulator_events"] = list(self.event_parser.events)
        return metrics
...
def get_model(usr_args: Dict[str, Any]):
    ...
    policy = WorldActionRobotWinPolicy(
        ...
        repair_skill_config=OmegaConf.to_container(
