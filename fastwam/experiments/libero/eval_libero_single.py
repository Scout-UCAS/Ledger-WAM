...
def _predict_action_chunk(
    ...
    *,
    ...
    input_h: int,
    model_device: str,
    event_parser: Optional[SimulatorEventParser] = None,
    ...
    if event_parser is not None and hasattr(model, "ledger"):
        ledger_evidence, _ = event_parser.observe(
            obs,
            image=_frame_to_rgb_array(imgs),
        )
        infer_kwargs["ledger_evidence"] = ledger_evidence
    ...
    with torch.no_grad():
        if visualize_future_video:
            joint_pred = model.infer_joint(**infer_kwargs)
            predicted_future_frames = _select_predicted_future_frames(
                joint_pred["video"], cfg
            )
            if hasattr(model, "ledger"):
                action_infer_kwargs = dict(infer_kwargs)
                action_infer_kwargs.pop("num_video_frames", None)
    ...
    execution_horizon = int(pred.get("execution_horizon", action.shape[0]))
    execution_horizon = max(1, min(execution_horizon, action.shape[0]))
    ...
    planner = pred.get("planner", {})
    action = apply_cartesian_repair(
        action,
        planner.get("repair_name"),
        config=OmegaConf.to_container(
            cfg.EVALUATION.get("repair_skills", {}), resolve=True
        ),
    )
    if event_parser is not None:
        event_parser.last_action = np.asarray(action[0], dtype=np.float32)
...
def run_single_episode(
    ...
    *,
    ...
    env.reset()
    if hasattr(model, "reset_ledger"):
        model.reset_ledger()
    event_parser = SimulatorEventParser()
    ...
    ledger_metrics = None
    if hasattr(model, "get_ledger_episode_metrics"):
        ledger_metrics = model.get_ledger_episode_metrics(reset=False)
        ledger_metrics["simulator_events"] = list(event_parser.events)
    return bool(done), replay_images, predicted_future_video_clips, episode_mean_psnr, ledger_metrics
...
def run_single_task(
    ...
    *,
    ...
    if hasattr(model, "get_ledger_episode_metrics"):
        results["episode_ledger_metrics"] = []

    for trial_idx in range(int(cfg.EVALUATION.num_trials)):
        success, replay_images, predicted_future_video_clips, episode_mean_psnr, ledger_metrics = run_single_episode(
            env=env,
        ...
        if ledger_metrics is not None:
            results["episode_ledger_metrics"].append(ledger_metrics)
    ...
    if "episode_ledger_metrics" in results:
        ledger_metrics = results["episode_ledger_metrics"]
        results["ledger_repair_rate"] = float(
            sum(item["num_repairs"] for item in ledger_metrics)
            / max(sum(item["num_replans"] for item in ledger_metrics), 1)
        )
        results["ledger_mean_rollbacks"] = float(
            np.mean([item["num_rollbacks"] for item in ledger_metrics])
        )
        results["ledger_mean_global_risk"] = float(
            np.mean([item["mean_global_risk"] for item in ledger_metrics])
        )
    return results
