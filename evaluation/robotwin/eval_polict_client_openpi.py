...
robowin_root: Optional[Path] = None
CONFIGS_PATH = None
UnStableError = None
Sapien_TEST = None


def _resolve_robowin_root(root_override: Optional[str] = None) -> Path:
    candidates = []
    if root_override:
        candidates.append(Path(root_override).expanduser())
    env_root = os.environ.get("ROBOWIN_ROOT")
    if env_root:
        candidates.append(Path(env_root).expanduser())
    cwd = Path.cwd()
    if (cwd / "envs").is_dir() and (cwd / "task_config").is_dir():
    ...
    configured = root_override or env_root
    if configured:
        raise RuntimeError(
            "RoboTwin root does not exist or is not a directory: {!r}".format(
                configured
            )
        )
    raise RuntimeError(
        "RoboTwin root is not configured. Pass --robowin_root, set "
        "ROBOWIN_ROOT, add robowin_root to the deploy config, or run this "
        "client from the RoboTwin repository root."
    )


def _load_robowin_environment(root_override: Optional[str] = None) -> Path:
    """Load RoboTwin imports only after resolving a valid installation root."""

    global robowin_root, CONFIGS_PATH, UnStableError, Sapien_TEST
    if CONFIGS_PATH is not None:
        return robowin_root  # type: ignore[return-value]

    configured_root = _resolve_robowin_root(root_override)

    robowin_root = configured_root
    if str(robowin_root) not in sys.path:
        sys.path.insert(0, str(robowin_root))
    os.chdir(robowin_root)

    from envs import CONFIGS_PATH as robowin_configs_path
    from envs.utils.create_actor import UnStableError as robowin_unstable_error
    from evaluation.robotwin.test_render import Sapien_TEST as robowin_sapien_test

    CONFIGS_PATH = robowin_configs_path
    UnStableError = robowin_unstable_error
    Sapien_TEST = robowin_sapien_test
    return robowin_root
...
from wan_va.ledger import repair_execution_ack_required


def _require_executable_action(response):
    """Return a valid action tensor or fail without inventing a repair action."""
    action = response.get("action")
    if action is None:
        action_id = response.get("repair_action_id", "unknown")
        if response.get("requires_repair_action"):
            raise RuntimeError(
                "Ledger-WAM selected repair action {!r}, but the server has no "
                "executable repair chunk. Configure repair_action_chunks on the "
                "controller; the RoboTwin evaluator will not fabricate one.".format(
                    action_id
                )
            )
        if repair_execution_ack_required(response):
            raise RuntimeError(
                "Ledger-WAM is waiting for repair_execution_ack for action "
                "{!r}; no new action can be executed.".format(action_id)
            )
        raise RuntimeError("Inference server returned no executable action")
    action = np.asarray(action)
    if action.ndim != 3:
        raise RuntimeError(
            "Inference action must have shape [channels, frames, actions_per_frame], "
            "got {}".format(action.shape)
        )
    return action


def _repair_execution_ack(response):
    if not repair_execution_ack_required(response):
        return None
    action_id = response.get("repair_action_id")
    if not action_id:
        raise RuntimeError(
            "Repair response requires an acknowledgement but has no action_id"
        )
    ...
    if not execution_id:
        raise RuntimeError(
            "Repair response requires an acknowledgement but has no execution_id"
        )
    return {
...
def get_camera_config(camera_type):
    if CONFIGS_PATH is None:
        raise RuntimeError("RoboTwin environment is not initialized.")
    camera_config_path = os.path.join(robowin_root, "task_config/_camera_config.yml")

    if not os.path.isfile(camera_config_path):
        raise FileNotFoundError(
            "RoboTwin task_config file missing: {!r}".format(camera_config_path)
        )
...
def main(usr_args):
    _load_robowin_environment(usr_args.get("robowin_root"))
    ...
    def get_embodiment_file(embodiment_type):
        ...
        if robot_file is None:
            raise ValueError("No embodiment files configured for {}".format(embodiment_type))
        return robot_file
...
def _normalize_quaternion(quat, label):
    norm = np.linalg.norm(quat)
    if not np.isfinite(norm) or norm <= 1e-8:
        raise RuntimeError("{} quaternion is invalid: {}".format(label, quat))
    return quat / norm


def _robotwin_ee_action_from_model_step(action_step, initial_eef_pose):
    action_step = np.asarray(action_step, dtype=np.float64).reshape(-1)
    if not np.isfinite(action_step).all():
        raise RuntimeError("RoboTwin action contains non-finite values")
    if action_step.shape[0] == 14:
        return np.concatenate([
            action_step[:3],
            euler2quat(action_step[3], action_step[4], action_step[5]),
            action_step[6:10],
            euler2quat(action_step[10], action_step[11], action_step[12]),
            action_step[13:14],
        ])
    if action_step.shape[0] == 16:
        ee_action = add_init_pose(action_step, initial_eef_pose)
        return np.concatenate([
            ee_action[:3],
            _normalize_quaternion(ee_action[3:7], "left"),
            ee_action[7:11],
            _normalize_quaternion(ee_action[11:15], "right"),
            ee_action[15:16],
        ])
    raise RuntimeError(
        "Unsupported RoboTwin action channel count {}. Expected 14 "
        "(dual-arm xyz+rpy+gripper) or 16 (dual-arm xyz+quat+gripper).".format(
            action_step.shape[0]
        )
    )
...
def eval_policy(task_name,
    ...
    while succ_seed < test_num:
        ...
        while TASK_ENV.take_action_cnt<TASK_ENV.step_lim:
            ...
            action = _require_executable_action(ret)
            repair_ack = _repair_execution_ack(ret)
            if 'video' in ret:
            ...
            if action.shape[2] % 4 != 0:
                raise RuntimeError(
                    "RoboTwin action count must be divisible by 4, got {}".format(
                        action.shape[2]
                    )
                )
            ...
            for i in range(start_idx, action.shape[1]):
                for j in range(action.shape[2]):
                    ...
                    ee_action = _robotwin_ee_action_from_model_step(
                        action[:, i, j], inint_eef_pose
                    )
            ...
            if repair_ack is not None:
                cache_request["repair_execution_ack"] = repair_ack
            model.infer(cache_request)
...
def parse_args_and_config():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the RoboTwin repository root.",
    )
    ...
    root_hint = args.robowin_root or os.environ.get("ROBOWIN_ROOT")
    if root_hint or ((Path.cwd() / "envs").is_dir() and (Path.cwd() / "task_config").is_dir()):
        _load_robowin_environment(root_hint)
    ...
    def parse_override_pairs(pairs):
        if len(pairs) % 2 != 0:
            raise ValueError("Overrides must come in --key value pairs.")
        override_dict = {}
    ...
    config.setdefault("robowin_root", args.robowin_root)
    if CONFIGS_PATH is None:
        _load_robowin_environment(config.get("robowin_root"))
